...
float cloud1X = -1.0f, cloud2X = 0.0f, cloud3X = 1.0f; // Scaled by 2
float cloudSpeed = 0.0006f; // Speed of cloud movement
// Function to draw the tube well with proper alignment in ortho range -2 to 2
...
void drawCloud(float x, float y, float size) {
    drawCircle(x, y, size, 1.0f, 1.0f, 1.0f);       // Center
    drawCircle(x - size / 1.5f, y, size / 1.5f, 1.0f, 1.0f, 1.0f); // Left
    drawCircle(x + size / 1.5f, y, size / 1.5f, 1.0f, 1.0f, 1.0f); // Right
    drawCircle(x, y + size / 2, size / 1.5f, 1.0f, 1.0f, 1.0f);    // Top
}
...
void drawTree(float x, float y, float trunkWidth, float trunkHeight, float foliageRadius) {
    ...
    // Draw foliage
    drawCircle(x, y + trunkHeight, foliageRadius, 0.0f, 0.8f, 0.0f);          // Center
    drawCircle(x - foliageRadius / 1.5f, y + trunkHeight - foliageRadius / 2, foliageRadius / 1.5f, 0.0f, 0.8f, 0.0f); // Left
    drawCircle(x + foliageRadius / 1.5f, y + trunkHeight - foliageRadius / 2, foliageRadius / 1.5f, 0.0f, 0.8f, 0.0f); // Right
}

void drawSchool() {
    // First Floor (Main building)
    glColor3f(0.9f, 0.7f, 0.5f); // Light brown
    glBegin(GL_QUADS);
    glVertex2f(-1.0f, -1.0f); // Scaled by 2
    glVertex2f(1.0f, -1.0f);  // Scaled by 2
    glVertex2f(1.0f, -0.4f);  // Scaled by 2
    glVertex2f(-1.0f, -0.4f); // Scaled by 2
    glEnd();

    // Terrace-like Border between First and Second Floor
    glColor3f(0.6f, 0.4f, 0.3f); // Brownish color
    glBegin(GL_QUADS);
    glVertex2f(-1.1f, -0.4f); // Scaled by 2
    glVertex2f(1.1f, -0.4f);  // Scaled by 2
    glVertex2f(1.1f, -0.36f); // Scaled by 2
    glVertex2f(-1.1f, -0.36f); // Scaled by 2
    glEnd();

    // Second Floor
    glColor3f(0.9f, 0.8f, 0.6f); // Slightly lighter brown
    glBegin(GL_QUADS);
    glVertex2f(-1.0f, -0.36f); // Scaled by 2
    glVertex2f(1.0f, -0.36f);  // Scaled by 2
    glVertex2f(1.0f, 0.2f);    // Scaled by 2
    glVertex2f(-1.0f, 0.2f);   // Scaled by 2
    glEnd();

    // Flat Terrace
    glColor3f(0.8f, 0.3f, 0.2f); // Red (terrace)
    glBegin(GL_QUADS);
    glVertex2f(-1.1f, 0.2f); // Scaled by 2
    glVertex2f(1.1f, 0.2f);  // Scaled by 2
    glVertex2f(1.1f, 0.3f);  // Scaled by 2
    glVertex2f(-1.1f, 0.3f); // Scaled by 2
    glEnd();

    // First Floor Door
    glColor3f(0.7f, 0.f, 0.1f); // Black
    glBegin(GL_QUADS);
    glVertex2f(-0.2f, -1.0f); // Scaled by 2
    glVertex2f(0.2f, -1.0f);  // Scaled by 2
    glVertex2f(0.2f, -0.7f);  // Scaled by 2
    glVertex2f(-0.2f, -0.7f); // Scaled by 2
    glEnd();
    ...
    glBegin(GL_QUADS);
    // Left window
    glVertex2f(-0.8f, -0.7f); // Scaled by 2
    glVertex2f(-0.6f, -0.7f); // Scaled by 2
    glVertex2f(-0.6f, -0.5f); // Scaled by 2
    glVertex2f(-0.8f, -0.5f); // Scaled by 2

    // Center window
    glVertex2f(-0.1f, -0.7f); // Scaled by 2
    glVertex2f(0.1f, -0.7f);  // Scaled by 2
    glVertex2f(0.1f, -0.5f);  // Scaled by 2
    glVertex2f(-0.1f, -0.5f); // Scaled by 2
    ...
    glBegin(GL_QUADS);
    // Left window
    glVertex2f(-0.8f, -0.1f); // Scaled by 2
    glVertex2f(-0.6f, -0.1f); // Scaled by 2
    glVertex2f(-0.6f, 0.1f);  // Scaled by 2
    glVertex2f(-0.8f, 0.1f);  // Scaled by 2

    // Center window
    glVertex2f(-0.1f, -0.1f); // Scaled by 2
    glVertex2f(0.1f, -0.1f);  // Scaled by 2
    glVertex2f(0.1f, 0.1f);   // Scaled by 2
    glVertex2f(-0.1f, 0.1f);  // Scaled by 2
...
void drawFlag() {
    ...
    glBegin(GL_QUADS);
    glVertex2f(-1.4f, -1.0f); // Scaled by 2
    glVertex2f(-1.36f, -1.0f); // Scaled by 2
    glVertex2f(-1.36f, 0.6f);  // Scaled by 2
    glVertex2f(-1.4f, 0.6f);   // Scaled by 2
    glEnd();

    // Flag rectangle
    glColor3f(0.0f, 0.5f, 0.0f); // Dark green
    glBegin(GL_QUADS);
    glVertex2f(-1.36f, 0.4f); // Scaled by 2
    glVertex2f(-0.96f, 0.4f); // Scaled by 2
    glVertex2f(-0.96f, 0.6f); // Scaled by 2
    glVertex2f(-1.36f, 0.6f); // Scaled by 2
    glEnd();

    // Red circle
    drawCircle(-1.16f, 0.5f, 0.08f, 0.8f, 0.0f, 0.0f); // Scaled by 2
}
...
void drawRoad() {
    ...
    glBegin(GL_QUADS);
    glVertex2f(-2.0f, -2.0f); // Scaled by 2
    glVertex2f(2.0f, -2.0f);  // Scaled by 2
    glVertex2f(2.0f, -1.6f);  // Scaled by 2
    glVertex2f(-2.0f, -1.6f); // Scaled by 2
    glEnd();

    // White lane divider
    glColor3f(1.0f, 1.0f, 1.0f); // White
    for (float i = -2.0f; i < 2.0f; i += 0.4f) { // Scaled by 2
        glBegin(GL_QUADS);
        glVertex2f(i, -1.8f);      // Scaled by 2
        glVertex2f(i + 0.2f, -1.8f); // Scaled by 2
        glVertex2f(i + 0.2f, -1.74f); // Scaled by 2
        glVertex2f(i, -1.74f);      // Scaled by 2
        glEnd();
    }
}

// Function to draw the walkway
void drawWalkway() {
    glColor3f(0.75f, 0.75f, 0.75f); // Light gray
    glBegin(GL_QUADS);
    glVertex2f(-0.2f, -1.0f);  // Scaled by 2
    glVertex2f(0.2f, -1.0f);   // Scaled by 2
    glVertex2f(0.1f, -1.6f);   // Scaled by 2
    glVertex2f(-0.1f, -1.6f);  // Scaled by 2
    glEnd();
}

// Function to draw the swing set at a new position and smaller size
void drawSwingSet(float x, float y) {
    // Draw swing frame (2 vertical poles and a horizontal crossbar)
    glColor3f(0.5f, 0.5f, 0.5f); // Brown color for the frame
    glBegin(GL_QUADS); // Left pole
    glVertex2f(x - 0.06f, y); // Scaled by 2
    glVertex2f(x - 0.06f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.06f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.06f, y); // Scaled by 2
    glEnd();
    glBegin(GL_QUADS); // Right pole
    glVertex2f(x + 0.4f, y); // Scaled by 2
    glVertex2f(x + 0.4f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.46f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.46f, y); // Scaled by 2
    glEnd();
    ...
    glVertex2f(x - 0.06f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.46f, y + 0.8f); // Scaled by 2
    glVertex2f(x + 0.46f, y + 0.9f); // Scaled by 2
    glVertex2f(x - 0.06f, y + 0.9f); // Scaled by 2
    glEnd();

    // Swing seat (a small rectangle)
    glColor3f(0.8f, 0.6f, 0.3f); // Wooden color for the seat
    glBegin(GL_QUADS);
    glVertex2f(x + 0.14f, y + 0.2f); // Scaled by 2
    glVertex2f(x + 0.34f, y + 0.2f); // Scaled by 2
    glVertex2f(x + 0.34f, y + 0.3f); // Scaled by 2
    glVertex2f(x + 0.14f, y + 0.3f); // Scaled by 2
    glEnd();

    // Ropes (lines from the crossbar to the seat)
    glColor3f(0.3f, 0.3f, 0.3f); // Dark gray for ropes
    glBegin(GL_LINES);
    glVertex2f(x + 0.2f, y + 0.8f); // Left rope
    glVertex2f(x + 0.2f, y + 0.2f); // To seat
    glVertex2f(x + 0.3f, y + 0.8f); // Right rope
    glVertex2f(x + 0.3f, y + 0.2f); // To seat
    glEnd();
}
void drawupdown(float x, float y) {
    // Draw base of the tube well
    glColor3f(0.6f, 0.4f, 0.2f); // Brown for the base
    glBegin(GL_QUADS);
    glVertex2f(x - 0.1f, y);          // Adjusted for ortho2d(-2, 2, -2, 2)
    glVertex2f(x + 0.1f, y);          // Adjusted for ortho2d(-2, 2, -2, 2)
    glVertex2f(x + 0.1f, y - 0.1f);   // Adjusted for ortho2d(-2, 2, -2, 2)
    glVertex2f(x - 0.1f, y - 0.1f);   // Adjusted for ortho2d(-2, 2, -2, 2)
    glEnd();

    // Draw vertical pipe
    glColor3f(0.3f, 0.3f, 0.3f); // Gray for the pipe
    glBegin(GL_QUADS);
    glVertex2f(x - 0.04f, y + 0.16f); // Bottom left, adjusted
    glVertex2f(x + 0.04f, y + 0.16f); // Bottom right, adjusted
    glVertex2f(x + 0.04f, y + 0.4f);  // Top right, adjusted
    glVertex2f(x - 0.04f, y + 0.4f);  // Top left, adjusted
    glEnd();

    // Draw handle of the tube well
    glColor3f(0.0f, 0.3f, 0.9f); // Dark gray for the handle
    glBegin(GL_QUADS);
    glVertex2f(x - 0.6f, y + 0.3f);  // Left side of handle, adjusted
    glVertex2f(x + 0.6f, y + 0.3f);  // Right side of handle, adjusted
    glVertex2f(x + 0.6f, y + 0.34f); // Handle thickness, adjusted
    glVertex2f(x - 0.6f, y + 0.34f); // Handle thickness, adjusted
    glEnd();

    // Draw a small rectangle for water coming out of the pipe
    glColor3f(0.0f, 0.0f, 0.0f); // Blue for water
    glBegin(GL_QUADS);
    glVertex2f(x - 0.1f, y + 0.2f);  // Left side of water, adjusted
    glVertex2f(x + 0.1f, y + 0.2f);  // Right side of water, adjusted
    glVertex2f(x + 0.1f, y);         // Water bottom, adjusted
    glVertex2f(x - 0.1f, y);         // Water bottom, adjusted
    glEnd();
}
...
void displayVillage()
{
    ...
    // Sky gradient
    glBegin(GL_QUADS);
    glColor3f(0.53f, 0.81f, 0.92f); // Sky Blue (top)
    glVertex2f(-2.0f, 2.0f); // Scaled by 2
    glVertex2f(2.0f, 2.0f);  // Scaled by 2
    glColor3f(0.68f, 0.85f, 0.9f); // Lighter Blue (bottom)
    glVertex2f(2.0f, -2.0f); // Scaled by 2
    glVertex2f(-2.0f, -2.0f); // Scaled by 2
    glEnd();

    // Clouds
    drawCloud(cloud1X, 1.6f, 0.2f); // Scaled by 2
    drawCloud(cloud2X, 1.5f, 0.24f); // Scaled by 2
    drawCloud(cloud3X, 1.4f, 0.16f); // Scaled by 2
    ...
    if (cloud1X > 2.4f) cloud1X = -2.4f;
    if (cloud2X > 2.4f) cloud2X = -2.4f;
    if (cloud3X > 2.4f) cloud3X = -2.4f;
    ...
    glVertex2f(-2.0f, -2.0f); // Scaled by 2
    glVertex2f(2.0f, -2.0f);  // Scaled by 2
    glVertex2f(2.0f, -1.0f);  // Scaled by 2
    glVertex2f(-2.0f, -1.0f); // Scaled by 2
    glEnd();

    // Draw Sun
    drawCircle(1.4f, 1.4f, 0.4f, 1.0f, 0.9f, 0.0f); // Scaled by 2

    // Road, walkway, flag, trees, and school
    drawRoad();
    drawWalkway();
    drawFlag();
    drawTree(-1.8f, -1.0f, 0.1f, 0.6f, 0.2f); // Scaled by 2
    drawTree(1.8f, -1.0f, 0.1f, 0.6f, 0.2f);  // Scaled by 2
    drawTree(1.4f, -1.0f, 0.2f, 1.0f, 0.5f);  // Scaled by 2

    drawSchool();


    // Draw Swing Set beside the walkway and smaller size
    drawSwingSet(0.8f, -1.5f); // Scaled by 2
    drawupdown(-1.2f, -1.4f);
    glutSwapBuffers();
}
...
void handleKeypress(unsigned char key, int x, int y)
{
	switch (key)
	{
        case 'a': // Stop movement
            speed = 0.0f;
            break;

        case 'w': // Start movement
            speed = 0.01f;
            break;
        case 'c': // Switch to City mode
            isCity = true;
            isVillage = false;
            isSchool = false;
            speed = 0;
        ...
        case 'r':  // Start rain (only in City)
            if (isCity)
            {
                isRaining = true;
            }
        ...
        case 'n': // Switch to Night mode (only in Village & School)
            if (!isCity)
            {
                isDay = false;
            }
        ...
        case 'd': // Switch to Day mode (only in Village & School)
            if (!isCity)
            {
                isDay = true;
            }
    ...
    glutPostRedisplay(); // Update the display after keypress
}
